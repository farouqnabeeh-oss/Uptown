              ...
              categories.forEach(cat => {
                ...
                prods.forEach(p => {
                    ...
                    content += '<div class="up-card" onclick="window.viewP(' + p.id + ')">' +
                        ...
                        '<div class="up-footer"><div class="up-price-box"><span class="up-price-tag">' + price.toFixed(0) + currency + '</span>' + (p.discount > 0 ? '<span class="up-price-old">' + p.basePrice + currency + '</span>' : '') + '</div><button class="up-add-pill">' + (isAr ? "أضف للسلة" : "Add to Cart") + '</button></div>' +
                        '</div></div>';
                });
                content += "</div></div>";
              });
